...
interface MesojetTabProps {
  clientId: string;
  centerId: string;
}

const MesojetTab: React.FC<MesojetTabProps> = ({ clientId, centerId }) => {
  ...
  const fetchSessions = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await getSessions(clientId, centerId, 'mesojet');
      setSessions(data);
    } catch (err) {
      console.error('Error fetching mesojet sessions:', err);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Error saving mesojet session:', error);
