    ...
    <div style={{ fontFamily: "'Raleway', sans-serif", minHeight: "100vh", background: "linear-gradient(135deg, #f0fdfa 0%, #fdf2f8 50%, #eff6ff 100%)" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        .hdr-title { font-family: 'Cormorant Garamond', serif; }
        input[type=number]::-webkit-inner-spin-button,
        input[type=number]::-webkit-outer-spin-button { -webkit-appearance: none; }
        input[type=number] { -moz-appearance: textfield; }
        .pill-btn { cursor: pointer; transition: all .2s ease; }
        .pill-btn:hover { transform: translateY(-1px); }
        .ech-card { transition: all .3s ease; }
        .ech-card:hover { transform: translateY(-3px); box-shadow: 0 12px 32px rgba(0,0,0,.12); }
        .counter-btn { cursor: pointer; user-select: none; transition: background .15s; }
        .counter-btn:hover { filter: brightness(.9); }
        .section-header { cursor: pointer; transition: background .15s; }
        .section-header:hover { filter: brightness(.97); }
        .fade-in { animation: fadeIn .4s ease; }
        @keyframes fadeIn { from { opacity:0; transform:translateY(8px); } to { opacity:1; transform:translateY(0); } }
        .total-glow { box-shadow: 0 0 0 3px rgba(13,148,136,.15), 0 8px 24px rgba(13,148,136,.2); }
      `}</style>

      {/* HEADER */}
      <header style={{ background: "linear-gradient(135deg, #0d9488 0%, #0891b2 50%, #be185d 100%)", padding: "24px 32px", display: "flex", alignItems: "center", justifyContent: "space-between", boxShadow: "0 4px 20px rgba(0,0,0,.15)" }}>
        <div>
      ...
      <div style={{ maxWidth: 1300, margin: "0 auto", padding: "24px 20px", display: "grid", gridTemplateColumns: "1fr 380px", gap: 24, alignItems: "start" }}>
            ...
            return (
              <div key={cat.id} style={{ background: "white", borderRadius: 16, marginBottom: 14, boxShadow: "0 2px 12px rgba(0,0,0,.06)", overflow: "hidden" }}>
                ...
                <div
                  ...
                  style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 20px", background: isOpen ? "white" : "white", borderLeft: `4px solid ${cat.color}` }}
                >
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    ...
                    <span style={{ color: "#9ca3af", fontSize: 18, transform: isOpen ? "rotate(180deg)" : "rotate(0deg)", transition: "transform .25s", display: "block" }}>▾</span>
                  </div>
                </div>
        ...
        <div style={{ position: "sticky", top: 20 }}>
          ...
          {/* Récapitulatif soins sélectionnés */}
