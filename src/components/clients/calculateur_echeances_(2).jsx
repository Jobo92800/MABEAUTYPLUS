...
// Distribution par nombre de séances (anciens soins, logique Excel exacte)
...
// Distribution d'un montant en euros (forfaits Mésojet)
// Premières échéances = ⌈total/N⌉, suivantes = ⌊total/N⌋ → somme exacte garantie
...
function computeTotal(inputs) {
  let total = 0;
  CATEGORIES.forEach(cat => cat.items.forEach(it => { total += (Number(inputs[it.key])||0) * it.price; }));
  return total;
}

function computeInstallments(inputs, N) {
  const v = k => Number(inputs[k])||0;
  ...
  // Fixes 1ère échéance : Guide, Tenue, Compléments
  payments[0] += v("B6")*19 + v("B7")*60 + (v("B42")+v("B43")+v("B44")+v("B45"))*37;

  // Anciens soins → distribution par sessions
  [
  ...
  // Forfaits Mésojet → distribution du montant en euros
...
export default function App() {
  ...
  const [openSections, setOpenSections] = useState({ corpo:true });
  ...
  return (
    <div style={{ fontFamily:"'Raleway',sans-serif", minHeight:"100vh", background:"linear-gradient(135deg,#f0fdfa 0%,#fdf2f8 50%,#eff6ff 100%)" }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        .hdr{font-family:'Cormorant Garamond',serif}
        input[type=number]::-webkit-inner-spin-button,input[type=number]::-webkit-outer-spin-button{-webkit-appearance:none}
        input[type=number]{-moz-appearance:textfield}
        .pb{cursor:pointer;transition:all .2s}
        .pb:hover{transform:translateY(-1px)}
        .ec{transition:all .3s}
        .ec:hover{transform:translateY(-3px);box-shadow:0 12px 32px rgba(0,0,0,.12)}
        .cb{cursor:pointer;user-select:none;transition:background .15s}
        .cb:hover{filter:brightness(.88)}
        .sh{cursor:pointer;transition:all .15s}
        .sh:hover{filter:brightness(.97)}
        .fi{animation:fi .35s ease}
        @keyframes fi{from{opacity:0;transform:translateY(6px)}to{opacity:1;transform:translateY(0)}}
        .tg{box-shadow:0 0 0 3px rgba(13,148,136,.15),0 8px 24px rgba(13,148,136,.2)}
        .tp{display:inline-block;background:#dbeafe;color:#1d4ed8;font-size:10px;font-weight:700;letter-spacing:.06em;padding:2px 7px;border-radius:6px;text-transform:uppercase;margin-left:6px}
        .ts{display:inline-block;background:#dcfce7;color:#15803d;font-size:10px;font-weight:700;letter-spacing:.06em;padding:2px 7px;border-radius:6px;text-transform:uppercase;margin-left:6px}
        input::placeholder{color:#c4c4c4}
        ::-webkit-scrollbar{width:4px}::-webkit-scrollbar-thumb{background:#e5e7eb;border-radius:2px}
      `}</style>

      {/* HEADER */}
      <header style={{ background:"linear-gradient(135deg,#0d9488 0%,#0891b2 50%,#be185d 100%)", padding:"22px 32px", display:"flex", alignItems:"center", justifyContent:"space-between", boxShadow:"0 4px 20px rgba(0,0,0,.15)" }}>
        <div>
        ...
        <div style={{ display:"flex", gap:10, alignItems:"center" }}>
          ...
          <button onClick={reset} style={{ background:"rgba(255,255,255,.2)", border:"1px solid rgba(255,255,255,.4)", color:"white", padding:"8px 18px", borderRadius:20, fontSize:13, cursor:"pointer", letterSpacing:".05em", fontFamily:"inherit" }}>Réinitialiser</button>
        </div>
      </header>
